...
export class Peer extends EventEmitter {
  ...
  constructor({
    ...
    logger.debug("Peer constructor [id: %s]", id);
  ...
  public routerReset(initial = false): void {
    ...
    this.routerReady = safePromise<Router>(
      new Promise<Router>((resolve, reject) => {
  ...
  private startTurnCredentialsRefresh(): void {
    this.turnCredentialsInterval = setInterval(async () => {
      ...
      if (error)
        return logger.error(
          "startTurnCredentialsRefresh() | failed to get router [error: %o]",
          error
        );

      const {
        mediaNode: { turnHostname, turnports, secret },
      } = router;

      if (turnHostname)
        iceServers = getIceServers({
          hostname: turnHostname,
          turnports: turnports,
          ...getCredentials(this.id, secret, 3600),
        });
  ...
  public set sessionId(sessionId: string) {
    ...
    if (oldSessionId !== sessionId) this.emit("sessionIdChanged", sessionId);
  }
  ...
  public set permissions(value: string[]) {
    ...
    added.forEach((permission) =>
      this.notify({ method: "permissionAdded", data: { permission } })
    );
    removed.forEach((permission) =>
      this.notify({ method: "permissionRemoved", data: { permission } })
    );

    if (removed.includes(Permission.SHARE_AUDIO)) {
      this.producers.forEach((p) => {
        if (
          p.appData.source === MediaSourceType.MIC ||
          p.appData.source === MediaSourceType.SCREENAUDIO
        )
          p.close();
      });
    }
    ...
    if (removed.includes(Permission.SHARE_VIDEO)) {
      this.producers.forEach((p) => {
        if (p.appData.source === MediaSourceType.WEBCAM) p.close();
      });
    }

    if (removed.includes(Permission.SHARE_SCREEN)) {
      this.producers.forEach((p) => {
        if (p.appData.source === MediaSourceType.SCREEN) p.close();
      });
    }

    if (removed.includes(Permission.SHARE_EXTRA_VIDEO)) {
      this.producers.forEach((p) => {
        if (p.appData.source === MediaSourceType.EXTRAVIDEO) p.close();
      });
    }
  }
  ...
  public addConnection(connection: BaseConnection): void {
    ...
    connection.on("request", async (request, respond, reject) => {
      try {
        ...
        if (context.handled) respond(context.response);
        else {
          logger.debug(
            "request() unhandled request [method: %s]",
            request.method
          );
    ...
    connection.once("close", () => {
      ...
      if (this.connections.length === 0) this.close();
    });
  }
  ...
  public getAddress(): clientAddress {
    const connection = this.connections
      .items[0] as unknown as IOServerConnection;
